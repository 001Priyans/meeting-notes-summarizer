...
function buildPrompt(transcriptText: string, customPrompt: string): string {
...
export async function summarizeTranscript({ transcriptText, customPrompt }: SummarizeParams): Promise<SummarizeResponse> {
  // Check if transcript has substantial content
  if (!transcriptText.trim()) {
    return { summary: 'No substantial content found.' };
  }
  ...
  try {
    ...
    // Build the prompt
    const prompt = buildPrompt(transcriptText, customPrompt || 'Provide a structured summary');
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('SAFETY')) {
        throw new Error('Content filtered by safety settings. Please try with different content.');
      }
    }
